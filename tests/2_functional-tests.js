...
suite('Functional Tests', function () {
  ...
  suite('Integration tests with chai-http', function () {
    ...
    // #2
    test('?name=SampleName', function (done) {
      chai
        .request(server)
        .get('/hello?name=SampleName')
        .end(function (err, res) {
          assert.equal(res.status, 200);
          assert.equal(res.text, 'hello SampleName');
          done();
        });
    });
    // #3
    test('{surname: "Colombo"}', function (done) {
      chai
        .request(server)
        .put('/travellers')
  ...
  suite('"Famous Italian Explorers" form', function () {
    ...
    test('Submit the surname "Colombo" in the HTML form', function (done) {
      browser.fill('surname', 'Cristoforo').pressButton('submit', () => {
        ...
        brower.assert.elements('span#dates', 1);
        done();
      })
    });
    // #6
